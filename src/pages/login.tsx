...
import type { SignIn as LoginType } from '../types/auth';
      ...
      >
        ...
        <div className="glass-panel p-8 rounded-3xl shadow-2xl border border-white/10 relative overflow-hidden bg-[#09090b]/40 backdrop-blur-xl">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 relative z-10">
            ...
            <button
              type="submit"
              className="w-full py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 bg-white text-black hover:bg-zinc-200 transition-all hover:scale-[1.02] shadow-xl mt-4"
            >
              {isPending ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-gray-900"></div> : <>Sign In <ArrowRight size={18} /></>}
            </button>
          </form>
        ...
        <p className="text-center mt-8 text-zinc-400 text-sm">
          ...
          <Link to="/signup" className="text-white font-medium hover:text-indigo-400 transition-colors">
            Sign up for free
          </Link>
        </p>
      </motion.div>
